...
export class EsamiPrivacyPersonaleComponent implements OnInit {
  ...
  constructor(public dipendenteService: DipendentiService,
    ...
    docService.getByType("RegolamentoInterno").then((res: DocumentoDipendente[]) => {
      ...
      var reg = list[0];
      uploadService
        .downloadDocQuality(reg.filename, reg.type)
        .then((x) => {
  ...
  ngOnInit() {
    //this.docService.getByType("RegolamentoInterno").then((res: DocumentoDipendente[]) => {
    //  const list = res.sort((a, b) => {
    //    return new Date(b.dateupload).getTime() - new Date(a.dateupload).getTime();
    //  });
  ...
  async savePrivacy(doc: DocumentoDipendente) {
    ...
    this.docService
      ...
      .then((result: DocumentoDipendente) => {
        ...
        this.docsprivacyDataSource.data = this.docsprivacy.sort((a, b) => {
          return new Date(b.dateupload).getTime() - new Date(a.dateupload).getTime();
        });
